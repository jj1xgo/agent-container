...
- 2026-09-06、[PR #114](https://github.com/jj1xgo/agent-container/pull/114)（main `c2660da`）のClaude strong nested sandbox設計に対するSecurity acceptance gateを、merge・再build後の通常production image `localhost/agent-container:dev`（`8cfae6888ee9232295f29d7867795e2fb6e64732964d70bfa6f01d347c556c6c`、Claude Code 2.1.263）で、利用者のprivate terminalから`bin/agentctl run findsummits --agent claude`を1回実施しました。事前の`doctor`は`claude-managed-policy: valid`を含む全項目PASS（`enableWeakerNestedSandbox: false`がimageへ正しく配布されていることを確認）。`/sandbox`は「dangerouslyDisableSandbox is disabled by org policy — cannot bypass」を含むmanaged強制の説明を表示し、filesystem書き込みはallowlist限定でsensitive path（`.claude/settings.json`等）は同allowlist内でも拒否、読み取りはOAuth token fileを含む少数のdeny pathを除き許可、networkはfiltering proxy経由という記述でした。`/hooks`は既知のblock済みplugin hook（`superpowers@claude-plugins-official`のSessionStart、`findsummits`固有）1件のみ、`/mcp`は0件でした。sandbox内Bash toolからの`python3 -m agent_container.claude_security_probe`はexit 0で`oauth_token_visible=false`、`token_file_readable=false`、`parent_token_via_proc_readable=false`の3行だけを出力しました。通常終了後、`findsummits`のcontainerは0件、state root配下に残存socketなしを確認しています。sandbox固定・hooks／MCP・credential非露出probe・正常cleanupの4項目はPASSです。`agent-handover create`（global scrubが非credential環境変数を落とさないことの確認を兼ねる）と`agent-family issue create`のFamily intake gateは今回`not run`（既存weaker-mode imageで別途PASS済みの経路であり、strong mode固有の追加確認は次回実施）。`/proc`の数字entryがsandbox内processだけであることの直接確認（`ls /proc`のcount）も`not run`です。

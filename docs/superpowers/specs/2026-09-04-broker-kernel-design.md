...
| 6-3 | egressを乗せ替え | runtimeの`concurrency="thread"`、worker回収、失敗通知（`wait_failed`相当） | thread方式とtunnel予約という「broker固有の追加状態」をkernelがどう受けるかを、最大のgithubより前に小さいbrokerで決める |

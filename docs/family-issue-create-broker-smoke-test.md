...
|---|---|---|
| Local automated | PASS | 2026-09-04、commit `34b1476`。lint、Codex 44、container 976、socket 18、forced-unknown 4、unexpected skip 0、`git diff --check` PASS。2026-09-02のcommit `699420f`（Codex 21、container 953、socket 17）でもPASS済み。hostのumaskでcheckout時に664となるmanaged `CLAUDE.md`は期待metadataの644へ戻して実行する。 |
| Real Podman Codex path | PASS | 2026-09-04、Podman 5.8.6、local rootless、crun 1.28。commit `34b1476`から専用instrumented imageをbuildし、Codex／Claude両pathを含むPodman suite 14 tests、unexpected skip 0。single socket-file bind、pinned fd不在、ancestor sentinel拒否、actual conmon ancestry、duplicate denial、credential non-exposure、broker停止時のnonzeroとterminal cleanupを確認。2026-09-02のcommit `699420f`でもPASS済み。 |
| Real Podman Claude path | PASS | 同じ14-test実Podman suiteでClaude pathをegress有無の双方について確認。認証済み実Claude CLIによるintakeは次のIntake行で別に記録する。 |
...
| Intake / duplicate / non-exposure | PASS | Codex実runtime（2026-09-02）は固定fixture 1件をpendingへ保存し、同runの2回目をduplicateとして拒否。Claude実runtime（2026-09-04、Claude Code v2.1.258、`bin/agentctl --image localhost/agent-family-test:local run findsummits --agent claude`をhostのptyから起動）は、指示どおり`agent-family issue create`を実行して1回目が`pending`／request ID／expiryだけを出力し、同runの2回目はone-time capability消費済みとして`family intake request failed`（exit 1）で拒否された。hostのpreviewはcanonical title／bodyと末尾1回の`— Claude (findsummits)`署名を表示し、Claude由来requestはfresh approvalなしのためGitHub Issueを作成していない。実Podman suiteではCodex／Claude双方のenvironment、argv、mount、inspection、filesystemにcredential／repository／pending path／approval commandがないことを確認。2026-09-06、`/proc` unmask（#108）、Codex sandbox network（#109）、既存project profile更新（#110）の後、専用image `edd9916b52f3`と通常`run`経路の非対話Codex（`codex --approve-for-me exec`）で再確認し、Codex sandbox内から実行した`agent-family issue create`の1回目が`pending`（request `6b119494…`）、同runの2回目が拒否、auditは`intake`／`pending` 1行だけ増加、previewの署名は末尾1回であることを観測した。同日、同じ専用image `edd9916b52f3`と利用者のprivate terminalから起動した実Claude（Claude Code 2.1.261）のsandbox内Bash toolでも再確認し、security probeの3項目false（`parent_token_via_proc_readable=false`を含む）の後、`agent-family issue create`の1回目が`pending`（request `8e360eaa…`）、同runの2回目が`family intake request failed`（exit 1）、auditは`intake`／`pending`とpreviewの`preview`／`pending`の固定field 2行だけ増加、previewの`— Claude (findsummits)`署名は末尾1回でCodex署名なしであることを観測した。 |
| Approved real Issue | PASS | fresh approval後にrequest 1件だけを作成し、[Issue #75](https://github.com/jj1xgo/agent-container/issues/75)のtitle/body一致とOPENを確認。同requestの再approveは送信前に拒否され、terminal recordのcanonical contentは消去済み。2026-09-06、Codex sandbox内intake（request `6b119494…`）についてsection 5の6項目提示後に利用者が対話terminalでapproveし、[Issue #111](https://github.com/jj1xgo/agent-container/issues/111)のtitle一致・OPEN・author `app/jj1xgo-family-issue-broker`・署名末尾1回、host側state `created`、本文消去、audit `send`／`cleanup` 2行だけの追加を確認。TTYなしと確認文不一致の試行は`denied` auditだけで外部操作なし。 |
| Forced unknown / reconciliation | PASS | 2026-09-02、host-only fake fixture 4 tests PASS。network、Podman、GitHub mutationなし。 |
| Cleanup / rollback | PASS | Codex正常終了（2026-09-02）とClaude正常終了（2026-09-04）後、関連container 0、socket file 0、inspection marker 0。pending／audit validatorはPASS。auditは固定6 field、9 eventsで、Claude intakeは`intake`／`pending`と`preview`／`pending`の2 eventだけを追加し、本文やrepositoryを含まない。App／bindingは継続検証用に保持し、作成済みIssueは変更しないためrollbackはnot run。2026-09-06のClaude再確認（`/proc` unmask後）の通常終了後も、`findsummits`と専用imageのcontainerは全stateで0件、inspection marker 0、family intake directoryは既知の空directory 2件のまま、残存socketは別project（`agent-container`、2026-09-05起動）のhandover brokerのものだけで、rollbackは引き続きnot run（binding保持）。 |

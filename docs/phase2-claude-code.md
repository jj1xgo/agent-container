...
`allowManagedPermissionRulesOnly`は現在pinしていません。Claude Code 2.1.260ではこの設定がpermission promptの「Yes, and don't ask again」選択肢も無効にするため、pinするとcontainer内で保存したallow ruleが全て使えなくなり、session毎に再承認が必要になります。managed policyの`deny`、sandbox、bypass禁止、brokerのhost承認はpinの有無に関係なく効きます。中身を確認していない第三者repositoryをこのcontainerで動かす場合は、pinするか、`run`前にそのrepositoryの`.claude/settings.json`を確認してください。判断の経緯は[設計文書](superpowers/specs/2026-08-24-debian-project-images-claude-sandbox-design.md)を参照してください。

...
`main`を追随する開発版は通常どおりcloneします。正確なcheckout versionは
`bin/agentctl --version`で確認できます。
...
Codex、Claude Code、local developer、CIは同じ`bin/lint` wrapperを使います。標準agent imageにはpinned Ruff、pip、ripgrep (`rg`)、`jq`が入っているため、agent runtimeではlint、高速なcode検索、JSON操作をそのまま実行できます。
...
runtimeはrootless Podman、read-only root filesystem、capability削除、`no-new-privileges`、限定したmountを使用します。agent runtimeはagentのnested sandboxが`/proc`をmountできるようPodman既定の`/proc` maskだけを外し、`/sys`のmaskは維持します。外向きnetworkは既定ではdomain allowlistされていません。project単位のopt-inでexact-domain allowlistを有効にできます。containerは被害範囲を狭める境界であり、agentへ渡したcredentialの完全な秘密保持を保証するものではありません。
